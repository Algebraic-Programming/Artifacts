...
class Schedule:
    ...
    def validate(self):
        for source, target in self.instance.G.edges():
            ...
            if self.when[target] < self.when[source] + required_diff:
                print(f'Error in edge {source}-{target}: sup steps {self.when[source]} {self.when[target]}, procs {self.where[source]} {self.where[target]}')
                return False
    ...
    def compute_objective(self):
        ...
        #Wlist = {superstep: self.W[superstep] for superstep in self.S}
        #Clist = {superstep: self.instance.g * self.C[superstep] for superstep in self.S}
