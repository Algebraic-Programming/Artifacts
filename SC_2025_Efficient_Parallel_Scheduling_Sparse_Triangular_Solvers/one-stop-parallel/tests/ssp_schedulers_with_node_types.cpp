#define BOOST_TEST_MODULE SSP_SCHEDULERS_NODE_TYPE
...
void run_test2(SSPScheduler *test_scheduler) {
    // static_assert(std::is_base_of<Scheduler, T>::value, "Class is not a scheduler!");
    std::vector<std::string> filenames_graph = test_graphs();
    std::vector<std::string> filenames_architectures = test_architectures();
...
BOOST_AUTO_TEST_CASE(GreedyVarianceSspScheduler_test_2) {
   GreedyVarianceSspScheduler test;
   run_test2(&test);
}
...
BOOST_AUTO_TEST_CASE(GreedyVarianceSspScheduler_memory_test2) {
    GreedyVarianceSspScheduler test;
    test.setUseMemoryConstraint(true);
    run_test2(&test);
}
...
BOOST_AUTO_TEST_CASE(GreedySspLocking_test2) {
   GreedySspLocking test;
   run_test2(&test);
}